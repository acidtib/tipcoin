...
#if defined(NDEBUG)
# error "Tipcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Tipcoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s TIP additional fees, %d delta bytes\n",
